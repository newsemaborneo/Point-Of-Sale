<x-layouts.app title="Stok Masuk">
    <div class="space-y-6">
        <h1 class="text-2xl font-semibold text-slate-900">Form Stok Masuk</h1>
        <p class="text-sm text-slate-500">Isi formulir di bawah untuk mencatat penambahan stok produk.</p>

        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <form action="{{ route('stock.in') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                ...
                <button type="submit" class="rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-700">Simpan Stok Masuk</button>
            </form>
        </div>
    </div>
</x-layouts.app>